...
// Объявляем указатель на беззнаковый символ (байт),  который получает адрес области памяти, выделяемой функцией malloc, получающей на входе размер изображения '.fmt.pix.sizeimage' + длину текстовой строки, хранимой в массиве 'header' ( = P6\n640 480 255\n)
        ...
        memmove(asil, dst_buf, fmt.fmt.pix.sizeimage);                                  // Копируем (перемещаем?) блок в область на которую указывает 'asil', из области, на которую указывет 'dst_buf', длиной '.fmt.pix.sizeimage' (размер изображения в байтах)
        memmove(asil+qstrlen(header), asil, fmt.fmt.pix.sizeimage);                     // Копируем (перемещаем?) блок в область, на которую указывает 'asil' со смещением на длину текстовой строки,  хранимой в массиве 'header' ( = P6\n640 480 255\n), из области на которую указывает 'asil', длиной стрки в header
        memcpy(asil,header,qstrlen(header));                                            // Копируем (без промежуточного буфера) блок в область, на которую указывает 'asil', из области на начало которой указывает 'header', длиной строки в header
...
//                parent->img=qq;                                                       // Свойству 'img' предка (видеовиджета) присваиваем содержимое 'qq' (копируем в него данный кадр в полном размере)
        ...
        if(asil)                                                                        // Если указатель 'asil' true, освобождаем память выделенную под 'asil', но сам указатель при этом не меняется, то есть указывает на тот же блок памяти, а не на NULL
            free(asil);
//        msleep(40);                                                                   // Ограничиваем скорость захвата 25к/сек (40 мсек/кадр) ДОБАВЛЕНО 050419!
        msleep(200);                                                                    // Ограничиваем скорость захвата 5к/сек (200 мсек/кадр) ДОБАВЛЕНО 060519!
